...
namespace minicc {
    ...
    class IRGenerator : public ASTVisitor {
        ...
        std::map<Statement*, llvm::BasicBlock*> breakDestMap;
        std::map<Statement*, llvm::BasicBlock*> continueDestMap;

        void setExprValue(Expr* expr, llvm::Value* value);

        llvm::Value* getExprValue(Expr* expr);

        llvm::Type* getLLVMType(Type::PrimitiveTypeEnum type, size_t arraySize = 0);


        // Program* currProg = nullptr;

        void insertSyslibDeclarations();
